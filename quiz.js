...
function loadQuiz() {
    fetch("chat-data.json")
        ...
        .catch(err => {
            console.error("Failed to load chat-data.json:", err);
            document.getElementById("question-text").textContent = "Ошибка загрузки вопросов. Проверьте подключение к chat-data.json";
        });
}
...
function selectAnswer(index) {
    ...
    document.querySelectorAll("#options-container .option").forEach((el, i) => {
...
function showResults() {
    ...
    document.getElementById("score-percent").textContent = Math.round((score / selectedQuestions.length) * 100);
    document.getElementById("score-summary").textContent = `Вы набрали ${score} из ${selectedQuestions.length} баллов`;
